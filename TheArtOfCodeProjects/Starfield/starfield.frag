...
/**
	...
	Fixed the grid cells artefacts.

	It only works for the StarLayer. Perhaps, something about combining
	the several layers:

	This works the best. Don't know why yet.
	int trv = min(int(scale / 2), 1);

	the below doesn't work for some reason
	// int trv = 1; works just normally. Scale isn't even needed.
	But this is only the case when the the range of starfields being mixed
	are this way:
	float scale = mix(20., 0.5, depth);

	Though the artefacts appeared from time to time still.

	However doing
	float scale = mix(5.0, 2.0, depth);

	allows a scaling range that doesn't produce artefacts.
...
*/
vec3 StarLayer(in vec2 uv, float scale)
{
	vec3 col = vec3(0.0);
	...
	*/
	...
	/**
		...
		So the positions of those neighbouring star needs to be gotten
	*/
	//	could do int(floor(scale/2.0));
	//	But the below seems to be the best.
	int trv = min(int(scale / 2), 2);
	// int trv = min(int(scale / 2), 5);
	for (int y=-trv; y<=trv; y++)
	{
		for (int x=-trv; x<=trv; x++)
		{
			...
			*/
			...
			//	Same time fading
			// star *= sin(u_time * 3.0) * 0.5 + 0.5;

			//	Individual random fading
...
void DrawStarLayers()
{
	...
	vec2 M = norm_mouse(u_mouse).xy;//(norm_mouse(u_mouse).xy - u_resolution * 0.5) / u_resolution.y;
	
	float t = u_time * .02;	//	* 0.1 so it doesn't go too fast

	//	Doing this + M allows the look around effect
	uv += M * 4.0;
	...
	//	was originally `i += 1.0`.
	//	The below increases the brightness of each star, as it
	//	causes more summation of layer colors.
	for (float i=0.; i < 1.; i += 0.5 / NUM_LAYERS)
	{
		/**
			scale was originally 20.0 for star layers at the back, making them smaller
			and was 0.5 for those to the front.

			But I changed it to be from 5.0 to 2.0, because these scales work well enough
		...
		*/
		...
		//	was originally `mix(20.0, 0.5, depth)`
		float scale = mix(5., 2.0, depth);
		...
		/**
			...
			The fade value will be 1 when it is smaller than .9, but becomes zero
			once it become >= 1.0, causing it to fade out at 1.0 smoothly.
		*/
		...
		*/
		...
		//	Doing - M allows the parallax effect
		col += StarLayer(uv * scale + i * 453.2 - M, scale) * fade;
