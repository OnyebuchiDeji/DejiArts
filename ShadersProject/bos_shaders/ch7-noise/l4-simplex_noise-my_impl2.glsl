...
void SimplexGridWithNormalNoise()
{
    ...
    vec3 spos = simplexGrid(pos);

    //col = vec3( noise(spos.xy)*.5+.5 );

    // gl_FragColor = vec4(col, 1.0);
    gl_FragColor = vec4(spos, 1.0);
}

void SimplexGridWithSimplexNoise()
{
    vec2 st = gl_FragCoord.xy / u_resolution;
    st.x *= u_resolution.x/u_resolution.y;
    vec3 col = vec3(0.0);
    ...
    col = vec3( simplexNoise(spos.xy)*.5+.5 );
...
void main()
{
    //SimplexGridWithNormalNoise();
    SimplexGridWithSimplexNoise();
}